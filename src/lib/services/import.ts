...
export const IMPORT_TEMPLATES: Record<string, ImportTemplate> = {
  ...
  warehouses: {
    ...
    table: "warehouses",
    title: "Склады",
    description: "Дополнительные склады. Вид — «transit» или «other»; центральный, транзитный и склады-автомобили создаются системой.",
  ...
  teams: {
    ...
    table: "teams",
    title: "Бригады",
    description: "Названия и описания бригад; склад бригады — это закреплённый за ней автомобиль.",
  ...
  switch (entity) {
    ...
    case "warehouses": {
      for (let i = 0; i < rows.length; i++) {
        ...
        if (ex) { await db.update(warehouses).set({ name, address: g(r, "address") || null, ...(isActive !== undefined && !["team", "vehicle", "central"].includes(ex.kind) ? { isActive } : {}) }).where(eq(warehouses.id, ex.id)); res.updated++; }
