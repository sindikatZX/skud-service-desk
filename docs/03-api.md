...
`GET /api/v1/audit` — записи журнала. Право `audit.view`.
Параметры отбора: `from`, `to`, `actorId`, `action`, `entity`, `q`, `limit` (до 500),
`offset`. Ответ: `{ rows, total, actors }`, где `actors` — сотрудники, встречающиеся в
журнале (для фильтра). Записи создаются автоматически на каждый изменяющий запрос —
см. `docs/14-audit-log.md`.

Изменяющие запросы с чужим `Origin` отклоняются с кодом `CROSS_ORIGIN` (403).
