...
## Склад бригады — автомобиль

Запас бригады хранится в её машине, поэтому складом является автомобиль:

| Поле `warehouses` | Назначение |
|---|---|
| `kind = 'vehicle'` | склад-автомобиль; заводится автоматически для каждой машины |
| `vehicle_id` | ссылка на автомобиль (уникальна), название склада ведётся автопарком |
| `kind = 'team'` | устаревший склад бригады: остаётся ради старых данных и мигрируется в машину |

Бригада обращается к складу через активное закрепление (`vehicle_assignments.released_at is null`);
при передаче машины другой бригаде запас остаётся в кузове, а отметка «у бригады» на серийных
единицах (`equipment_units.team_id`, статус `at_team`) пересчитывается. Бригада без автомобиля
склада не имеет — операции с её остатками возвращают понятную ошибку.

## Журнал действий и отзыв сессий

`audit_log` — записи о действиях пользователей: время, исполнитель (id + копия имени и
роли), действие (`create|update|delete|operation|login|logout|denied`), сущность и её
номер, фраза для человека, метод/адрес/статус запроса, IP и подробности (`jsonb`).
Индексы: по времени, по исполнителю, по паре «сущность + номер». Таблица только
пополняется — API удаления записей нет. Подробнее: `docs/14-audit-log.md`.

`users.password_changed_at` — момент последней смены пароля. Сессия, выданная раньше этой
отметки, не принимается: смена пароля обрывает ранее выданные токены.
